...
function failClosed(message) {
  ...
  if (editor) editor.setAttribute("data-license-error", "sealed-wasm-denied");
...
async function loadSealedCore() {
  ...
  if (!ok) failClosed(`Sealed WASM core denied this runtime: ${result}`);
...
try {
  ...
  show("Sealed WASM runtime is ready.");
} catch (error) {
  show(`Sealed WASM runtime failed: ${error?.message || error}`, { sticky: true });
  throw error;
}
